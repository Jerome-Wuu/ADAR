...
def random_overlay(x, stage, dataset='places365_standard'):
...
def random_cutout(imgs, stage, min_cut=10, max_cut=30):
    """
        args:
...
def random_cutout_color(x, stage, min_cut=10, max_cut=30):
    """
        args:
        imgs: shape (B,C,H,W)
...
def random_grayscale(images, stage, p=.3):
    """
        args:
...
def random_color_jitter(imgs, stage=0):
    """ 
...
def random_blur(input, stage, ksize=13, sigma=5):
    if stage >= 4:
        ksize=7
        sigma=1
    elif stage >= 3:
        ...
        ksize=15
        sigma=9
    return kornia.filters.gaussian_blur2d(input.float(), (ksize, ksize), (sigma, sigma))  


def random_pepper(img, stage, SNR=0.9):
...
def random_translate(images, stage, return_random_idxs=False): 
    """
        args:
        images: torch.tensor shape (B,C,H,W)
        size: int,  Output size after translation, default 100
        return_random_idxs: bool, if True return translation params
    ...
    outs = torch.zeros((bs, channels, size, size), dtype=images.dtype, device=device)
    
    h1s = torch.randint(0, size - h + 1, (bs,))
    w1s = torch.randint(0, size - w + 1, (bs,))
    
    for out, img, h1, w1 in zip(outs, images, h1s, w1s):
        out[:, h1:h1 + h, w1:w1 + w] = img

    outs = F.interpolate(outs, size=orig_size, mode='bicubic', align_corners=False)
...
def random_shift(imgs, stage, pad=4):
...
def random_crop(x, stage=2, size=84, w1=None, h1=None, return_w1_h1=False):
