...
def parse_args():
    ...
    # agent
    parser.add_argument('--algorithm', default='alter', type=str)
    ...
    # auxiliary tasks (rad, curl, pad, soda)
    parser.add_argument('--aux_lr', default=3e-3, type=float)
    ...
    # adar
    parser.add_argument('--single_steps', default='10k', type=str)
    ...
    assert args.algorithm in {'sac', 'rad', 'curl', 'pad', 'soda', 'drq',
                              'svea', 'adar'}, f'specified algorithm "{args.algorithm}" is not supported'

    assert args.eval_mode in {'train', 'color_easy', 'color_hard', 'video_easy', 'video_hard', 'distracting_cs',
                              'none'}, f'specified mode "{args.eval_mode}" is not supported'
    ...
    args.single_steps = int(args.single_steps.replace('k', '000'))
